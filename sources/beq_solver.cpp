...
double FO1DM::calc_only_xf(const VecString &channels) {
...
double FO1DM::calc_yield(const double &x) {
    ...
    if (x < xf) {
        ...
        res = (1. + foc.del) * BI.yeq(x);
    }
    else {
        double yfo = (1. + foc.del) * BI.yeq(xf);
        if (appr) {
            FOAppr foa(BI);
            res = adap_gauss_kronrod_15(foa, xf, x, 1e-3);
            res = 1. / yfo - res;
            res = 1. / res;
        } else {
            FOFull fof(BI);
            Output out;
            Odeint<StepperDopr853<FOFull>> ode(yfo, xf, x, 0., 1e-6, 0.1, 0.,
                                               out, fof);
            ode.integrate();
            res = yfo;
        }
